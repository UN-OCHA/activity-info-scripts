...
class FormTree(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in forms (dict)
        _field_dict = {}
        if self.forms:
            for _key_forms in self.forms:
                if self.forms[_key_forms]:
                    _field_dict[_key_forms] = self.forms[_key_forms].to_dict()
            _dict['forms'] = _field_dict
        return _dict
